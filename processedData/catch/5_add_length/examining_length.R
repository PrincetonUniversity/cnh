...
with(subset(met_len, metier == "TWL_3"), plot(density(len,na.rm=T), col = "dodgerblue",xlim=c(0,300),bty = "n", main = "distribution of vessel length", xlab = "length (ft)", ylim = c(0,.13)))
with(subset(met_len, metier == "TWL_1"), lines(density(len,na.rm=T), col = "orange"))
with(subset(met_len, metier == "POT_1"), lines(density(len,na.rm=T), col = "red"))
with(subset(met_len, metier == "TLS_1"), lines(density(len,na.rm=T), col = "pink"))
with(subset(met_len, metier == "HKL_2"), lines(density(len,na.rm=T), col = "slategrey"))
with(subset(met_len, metier == "TWS_1"), lines(density(len,na.rm=T), col = "magenta"))
...
legend("topright",col = c("dodgerblue","orange","red","pink","slategrey"),lwd=3, legend = c("whiting midwater", "DTS trawl","Dungeness crab pot","chinook troll","black rockfish hook/line"), bty = 'n', cex = .5)
