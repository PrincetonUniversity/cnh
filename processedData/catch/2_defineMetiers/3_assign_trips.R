...
assign_trips <- function(ticket_data=filtered_ftl, 
  ...
  if(species_def=="modified"){
    melt_ftl <- select(sub_data, trip_id, modified, adj_revenue)
  ...
  ref_metier <- merge(ref_metier, training_list[,c("cluster","trip_id")], 
                      all.x = TRUE)  
  y = ref_metier$cluster
  
  # calculate distance from all points
  cat("calculate distance from all points\n")
  d <- as.matrix(vegdist(decostand(cast_ftl, "hellinger"), "euclidean")) 
