...
library(reshape2); library(permute); library(plyr); library(dplyr);library(vegan)

source("~/Documents/cnh/processedData/catch/2_defineMetiers/3_assign_trips.R")

#libloc = "/tigress/efuller/R_packages"

#args <- commandArgs(trailingOnly = TRUE) # if called from CL

args <- c(2012, "TWL", 2015, "spid")
...
class_year <- args[3] # year of data to classify: 2009, 2011, 2012, 2013, 2014, 2015
...
# (so some species catch has been removed)
  if(!exists("filtered_ftl")){
    filtered_ftl <- readRDS("~/Documents/cnh/processedData/catch/1_cleaningData/filtered_ftl.RDS")
  }
