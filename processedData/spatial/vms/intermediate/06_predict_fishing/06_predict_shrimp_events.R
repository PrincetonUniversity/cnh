# build predicting-out dataset for rest of shrimp trips
# generate identically processed vms data
...
load_vms <- function(window, downscale = TRUE){
  ...
  files <- dir(fp)
  dat_list <- list()
  # only load boats that have shrimp trips
  for(i in 1:length(files)){
    infile <- read.csv(paste0(fp,files[i]), stringsAsFactors = FALSE)
    if(any(infile$metier.2010=="TWS_1",na.rm=T)){
      if(any(colnames(infile)=="optional")){
        infile$optional <- NULL
      }
      dat_list[[i]] <- infile
    }else{
      dat_list[[i]] <- NA
  ...
  filter_dat_list <- dat_list[-which(is.na(dat_list))]
  dat_long <- do.call(rbind, filter_dat_list)
  
  # only keep points that are at sea (i.e. only_trips!=0) 
  # and drop lots of columns that are unnecessary
  vms_files <-  dat_long %>%
    filter(only_trips!=0, metier.2010=="TWS_1") %>%
...
vms_files <- load_vms(window = 24, downscale = TRUE)
...
saveRDS(list(loc_data = loc_data, 
             ...
             true_data = true_data),
        file="processedData/spatial/vms/intermediate/06_predict_fishing/06_obs_all_vms_dat.RDS")

# predict new vector of fishing ----
# remove NAs
...
library(rfUtilities)
rf_modlist <- readRDS("processedData/spatial/vms/intermediate/06_predict_fishing/06_rf_model_list.RDS")

pred_rf1 <-   predict(rf_modlist$rf_model, training_dat_norm, type = "prob")
colnames(pred_rf1) <- paste0("p",colnames(pred_rf1))
pred_rf1 <- as.data.frame(pred_rf1)
pred_rf1$true <- true_data
pred_rf1$pred <- ifelse(pred_rf1$p1>.5, 1, 0)

table(pred_rf1$true, pred_rf1$pred,deparse.level = 2)

vms_classed <- loc_data
vms_classed$predicted_fishing <- pred_rf1$pred
vms_classed$true_fishing <- pred_rf1$true

saveRDS(vms_classed, "processedData/spatial/vms/intermediate/06_predict_fishing/vms_classified.RDS")
