...
source("compile matrices (trips).R")
...
#dimnames(ar) <- list(c("control","reduce_POT1"),my.headers)
...
df.cor.crabd$Metier.proper <- c("Sablefish",
                                "Red sea urchin",
                                "Market squid",
                                "Pacific sardine",
                                "Dungeness crab",
...
my.order <- rev(sort(df.cor.crabd$Metier.proper))
df.cor.crabd$Metier.sort <- factor(df.cor.crabd$Metier.proper, levels=my.order)
df.cor.crabd.sort <- df.cor.crabd [order(df.cor.crabd$Metier.sort),]

ggplot(df.cor.crabd.sort, aes(x=Metier.sort, y=Rel.change.with.no.crab)) +
