...
duration <- seconds(as.duration(as.interval( time[2], tail(time,2)[1] )))
n_obs <- as.numeric(floor( duration / resolution ))
# TIME UNITS ???????

interp_result = as.data.frame(matrix(ncol=2, nrow=(n_obs) ) )
...
for (i in 1:(n - 3) ) {
	oneStep <- as.numeric(seconds(as.duration(as.interval(time[i], time[i+1]))))
	subindex <- leftover
	if (is.na(subindex)) cat("subindex NA")
	if (is.na(oneStep)) cat ("oneStep NA")
