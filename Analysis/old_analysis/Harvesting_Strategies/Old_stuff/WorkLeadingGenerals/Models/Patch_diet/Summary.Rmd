...
On optimal diet in a patchy environment: `r citep("Heller")`
-----------------------------------------------------------
`r citet("Heller")` simulates diet choice when a solitary individual can forage in two depletable patches that each have two different prey choices. Using a few different strategies he examines when maximizing the instaneous gain rate and the long-term intake differ. 


Current Simple Model --- extension of `r citet(c(Abrams="10.1890/11-0011.1"))`
------------------------------------------------------------------------------------
`r citet("Abrams")` uses a 2 patch predator-prey metapopulation model where harvesting only occurs on predators in one patch, prey do not move. The model is as follows:
...
Using parameters from `r citet("Abrams")`, figure 1, where possible

```{r echo=FALSE,results='markup',message=FALSE,warning=FALSE,wrap = 'div', out.extra="",fig.width=7,fig.height=5}
r=5 # birth rate (adjusted)
k=.7 # carrying capacity (adjusted)
d=0.02 # death rate
q=1    # catchability, not in Abrams et al. 2013
C = 0.5   # parameter that should not drive population to extinction
E = 0.25  # " "
R = -1:10
N = -1:10
b = .05 # (adjusted)
plot(rep((d+q*E/b*C),length(N)),N,lwd=2,col="red3",type='l',xlim=c(0,10),xlab="R",frame=FALSE)
abline(v=-0.05,lwd=3)
abline(h=-0.05,lwd=3)
lines(R,(r-k*R)/C,lwd=2,col="green4",ylab="N") # plotting nontrivial R nullcline
abline(v=0,lwd=2,col="green4") # plotting trivial R nullcline
abline(h=0,lwd=2,col="red3")
legend('topright',legend=c("Prey Nullcline","Pred Nullcline","Pred Nullcline when E=E_MSY"),col=c("green4","red3","red3"),lwd=2,lty=c(1,1,2),bty='n')
E_MSY=((r*b*C)/(2*k*q))-d/(2*q)
abline(v=d+q*E_MSY/b*C,lty=2,lwd=3,col="red3")
```
...
```{r echo=FALSE,results='markup',message=FALSE,warning=FALSE, wrap = 'div', out.extra="",fig.width=4, fig.height=4}

E = seq(-1,1,0.001)
Y = q*E*((r-k*((d+q*E)/b*C))/C)
plot(E,Y,'l',lwd=2,xlim=c(0,1),ylim=c(0,2),frame=FALSE)
```
...
```{r message=FALSE,warning=FALSE,fig.width=6,fig.align='center',fig.height=5,fig.width=5,out.extra='style="display:block;"'}
...
```
## Current point:
* I had to adjust the parameters quite a bit from `r citet("Abrams")` in order to get the predators to stay alive.
