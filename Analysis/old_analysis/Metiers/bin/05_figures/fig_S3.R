...
png("/Users/efuller/Desktop/CNH/Analysis/Metiers/bin/05_figures/fig_S3.png",width = 5, height = 5, units = "in", res = 500, bg= "transparent")
#image(k, col= colorRampPalette(rev(brewer.pal(9, "Greys")))(10000), bty = "n", ylab=expression(C[pre]))
plot( port_link$ic_pre,port_link$eff.shannon_2010,cex=.45, col="black",pch=3,bty="n",xlab=expression(C[pre]),  ylab = expression(H[pre]))
#abline(lm(eff.shannon_2010 ~ ic_pre, port_link))
dev.off()
