...
# load data
filtered_ftl <- readRDS("1_cleaningData/filtered_ftl.RDS")

# load metiers
files <- list.files(path="2_defineMetiers/classify_metiers/modified/2012/") # load 2012 predicted metiers
predicteds <- do.call(rbind, lapply(paste0("2_defineMetiers/classify_metiers/modified/2012/",files), readRDS))
other_files <- list.files(path="2_defineMetiers/define_metiers/")
other_files <- other_files[grep(2012, other_files)] # just want 2010 classified metiers

classifieds <- do.call(rbind, lapply(paste0("2_defineMetiers/define_metiers/", other_files), read.csv))
classifieds$X <- NULL
classifieds$ftid <- paste0(classifieds$ftid, 2012)
...
saveRDS(tickets, "3_analyzeMetiers/tickets.RDS")
#----
# characterizing data
#----
...
saveRDS(df, "3_analyzeMetiers/metier_descrp.RDS")
#----
