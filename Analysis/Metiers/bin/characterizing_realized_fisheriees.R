# characterizing realized fisheries

# plotting time series of trips/revenue/landings for 4 example fisheries ----
landings <- readRDS("/Users/efuller/1/CNH/processedData/catch/1_cleaningData/tickets.RDS")
library(dplyr)
fish_landings <- landings %>%
  group_by(tdate, metier.2010) %>%
  summarize(n.trips = length(unique(trip_id)), n.lbs = sum(landed_wt, na.rm = T), n.dollars = sum(adj_revenue, na.rm = T))

fish_landings$tdate <- as.Date(fish_landings$tdate, format = "%d-%B-%y")

sub_trips <- subset(fish_landings, metier.2010 %in% c("TLS_2", "TWS_1","HKL_1","POT_1","MSC_1"))
sub_trips$fishery <- ifelse(sub_trips$metier.2010 == "TLS_2", "albacore troll", 
                            ifelse(sub_trips$metier.2010 == "TWS_1", "pink shrimp trawl",
                                   ifelse(sub_trips$metier.2010 == "HKL_1", "sablefish long line", 
                                          ifelse(sub_trips$metier.2010 == "POT_1", "dungeness crab pot", 
                                                 ifelse(sub_trips$metier.2010 == "MSC_1", "red urchin diving",NA)))))

saveRDS(sub_trips, "/Users/efuller/1/CNH/Analysis/Metiers/bin/ex_trips.RDS")

# looking at latitudinal distribution of fisheries ----
# load port latitudes 
...
port_landings <- landings %>%
  group_by(pcid, metier.2010) %>%
  summarize(n.trips = length(unique(trip_id)), n.dollars = sum(adj_revenue, na.rm = T), n.lbs = sum(landed_wt, na.rm = T)) %>%
  left_join(ports)

library(grid)
sub_port <- subset(port_landings, metier.2010 %in% c("TLS_2","POT_1","TWS_1","HKL_1","MSC_1"))
  
  sub_port$fishery <- ifelse(sub_port$metier.2010 == "TLS_2", "albacore troll", 
                              ifelse(sub_port$metier.2010 == "TWS_1", "pink shrimp trawl",
                                     ifelse(sub_port$metier.2010 == "HKL_1", "sablefish long line", 
                                            ifelse(sub_port$metier.2010 == "POT_1", "dungeness crab pot", 
                                                   ifelse(sub_port$metier.2010 == "MSC_1", "red urchin diving",NA)))))

  
  saveRDS(sub_port, "/Users/efuller/1/CNH/Analysis/Metiers/bin/sub_port.RDS")

# generating table for realized fisheries 
  library(dplyr)
  library(stringr)
  library(vegan)
  library(reshape2)
