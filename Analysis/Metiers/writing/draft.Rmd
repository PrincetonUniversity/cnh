...
```{r, echo=FALSE,warning=FALSE,message=FALSE}
pred_data <- data.frame(cv.revenue = lm_pp$fitted.values, mean.diversity = yrdf[[2]]$mean_simpson, strategy = factor(yrdf[[2]]$single_cluster))
...
for(i in 1:length(strats)){
  ...
  if(i == 1){
    with(df, plot(mean.diversity, cv.revenue, ylim =c(0,.65), type = 'o', 
                  xlim = c(0,.7), pch = points[i], cex = .85, col = greys[i]))
    }else{
      with(df, points(mean.diversity, cv.revenue, ylim =c(0,.7), col = greys[i],
                  xlim = c(0,.7), pch = points[i], cex = .85, type = 'o'))
    }
}
legend(x = 0, y = .1, legend = strats, pch = points, horiz = FALSE, ncol = 5, col = greys)
```
