...
# remove vessels with veid == "********" and 0, and unknown
# should already be removed, so adding test functions
library(testthat)
expect_that(length(which(tickets$veid=="0")),equals(0))
expect_that(length(which(tickets$veid=="0")),equals(0))
expect_that(length(which(tickets$veid=="UNKNOWN")),equals(0))
expect_that(length(grep("[********]",tickets$veid)),equals(0))

# add grgroup to metier to distinguish between same numbers
tickets$metier <- paste(tickets$grgroup,tickets$metier,sep="_")

saveRDS(tickets, "code/3_analyzeMetiers/tickets.RDS")
#----
# characterizing data
#----
...
characterize_metiers <- function(metier_choice, data = tickets){
  ...
  catch_data <- subset(tickets, metier == metier_choice, select = c("modified","landed_wt", "trip_id","veid","ppp","drvid"))
...
saveRDS(df, "/Users/efuller/1/CNH/Analysis/Metiers/writing/code/3_analyzeMetiers/metier_descrp.RDS")
#----
...
# also morro bay in prep for michael bell conversation
years = 2009:2013
par(mfrow=c(2,3))
for(i in 1:length(years)){
  define_participationPlot(years[i], "MRO")
}

top_ports <- list(gs09NEW, gs09COS, gs09AST, gs09SF, gs09BFG, gs09WPT, gs09LWC, gs09BDG, gs09PRN, gs09TLL)
saveRDS(top_ports, "/Users/efuller/1/CNH/Analysis/Metiers/writing/code/3_analyzeMetiers/top_ports.RDS")
