...
# (so some species catch has been removed)
  if(!exists("filtered_ftl")){
    filtered_ftl <- readRDS("../define_metiers/filtered_ftl.RDS")
  }

# try for one gear type
if(spid_def=="modified"){
  train <- read.csv(paste0("../define_metiers/",grgroup,ref_year,".csv"),
                          stringsAsFactors=F)
...
# save output
if(spid_def=="modified"){
  saveRDS(predicted_trips, file=paste0("modified/",ref_year,"/",ref_year,"p",grgroup,class_year,".RDS"))
}else if(spid_def=="spid"){
