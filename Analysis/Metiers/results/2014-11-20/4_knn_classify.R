...
if(spid_def=="modified"){
  train <- read.csv(paste0("define/",grgroup,ref_year,"cluster_key.txt"),
                          stringsAsFactors=F)
...
# save output
if(spid_def=="modified"){
  saveRDS(predicted_trips, file=paste0(ref_year,"p",grgroup,class_year,".RDS"))
}else if(spid_def=="spid"){
